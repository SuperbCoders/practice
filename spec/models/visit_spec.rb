...
RSpec.describe Visit, type: :model do
  ...
  it 'is invalid without patinet if visit is signed' do
    visit = FactoryGirl.build(:visit, doctor: doctor, type: :signed,
        date: DateTime.now, duration: 60)

    expect(visit).to_not be_valid
  end

  it 'is invalid without unsigned_patient if visit is unsigned' do
    visit = FactoryGirl.build(:visit, doctor: doctor, type: :unsigned,
        date: DateTime.now, duration: 60)
  ...
  it 'can not be crossed with another visit' do
    visit = FactoryGirl.create(:visit, doctor: doctor, type: :unsigned,
        unsigned_patient: { name: Faker::Name.name },
        date: DateTime.now, duration: 60)

    second_visit = FactoryGirl.build(:visit, doctor: doctor, patient: patient,
        type: :signed, date: (DateTime.now + 20.minute), duration: 60)
